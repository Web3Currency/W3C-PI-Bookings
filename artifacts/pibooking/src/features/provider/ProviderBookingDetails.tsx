...
import { Calendar, Clock, Copy, Mail, Send, FileText, Hash, Paperclip, X } from 'lucide-react';
import { toast } from '../../hooks/use-toast';

interface ProviderBookingDetailsProps { booking: Booking; onClose: () => void; }

export const ProviderBookingDetails: React.FC<ProviderBookingDetailsProps> = ({ booking, onClose }) => {
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-3xl shadow-xl">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
            ...
            <div className="flex items-start gap-2"><Send className="w-3.5 h-3.5 mt-0.5 text-amber-600" /><div><span className="block text-[10px] font-bold uppercase text-zinc-400">Telegram</span><span className="font-mono font-bold text-zinc-900 break-all">{booking.clientPhone || 'Not provided'}</span></div></div>
