...
type FlowStep = 'browse' | 'about' | 'detail' | 'select_details' | 'review_summary' | 'payment' | 'status' | 'become_provider' | 'provider_console' | 'provider_services' | 'profile';
...
export default function App() {
  ...
  const { piUser, loading: piAuthLoading, signIn: signInWithPi, signOut: signOutPi, refreshProfile } = usePiAuth();
  ...
  const [clientDetails, setClientDetails] = useState<ClientDetails>({ clientName: '', clientPiUsername: '', clientPhone: '', clientEmail: '', notes: '' });
  ...
  const { isPulling, isRefreshing, pullDistance, pullProgress, isThresholdReached } = usePullToRefresh({ onRefresh: handleRefreshAll });
  ...
  useEffect(() => { if (!piUser) return; setClientDetails((prev) => ({ ...prev, clientPiUsername: piUser.globalUsername ? `@${piUser.globalUsername.replace(/^@/, '')}` : (piUser.username ? (piUser.username.startsWith('@') ? piUser.username : `@${piUser.username}`) : prev.clientPiUsername), })); }, [piUser]);
  ...
  const handleOpenProviderServices = () => { setActiveTab('browse'); setCurrentFlow('provider_services'); };
  const handleOpenProfile = () => { if (!piUser) return; setActiveTab('browse'); setCurrentFlow('profile'); };
  ...
  const handleConfirmDetails = (details: ClientDetails) => { setActiveTab('browse'); setClientDetails(details); setCurrentFlow('review_summary'); };
