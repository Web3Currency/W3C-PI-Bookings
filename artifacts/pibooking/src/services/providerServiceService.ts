...
async function request(path: string, init: RequestInit = {}) {
  const user = piAuthService.getStoredUser(); if (!user?.accessToken) throw new Error('Please sign in with Pi Network first.');
  ...
  const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || 'Provider service request failed.'); return body;
}
