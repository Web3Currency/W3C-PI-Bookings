  ...
  async getServicesAsync(): Promise<Service[]> {
    ...
    try {
      const [{ data, error }, { data: categoryRows, error: categoryError }] = await Promise.all([
        supabase.from('services').select('*, providers(*)').order('created_at', { ascending: true }),
        supabase.from('service_categories').select('id,name,slug').eq('is_active', true),
      ]);
