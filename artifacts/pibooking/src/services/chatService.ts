...
export const chatService = {
 async getConversations(): Promise<ChatConversation[]> { const data=await request<{conversations:ChatConversation[]}>('/pi/chat/conversations',authBody()); return data.conversations||[]; },
 ...
 async getMessages(conversationId:string):Promise<ChatMessage[]> { const data=await request<{messages:ChatMessage[]}>(`/pi/chat/conversations/${encodeURIComponent(conversationId)}/messages/list`,authBody()); return data.messages||[]; },
 async sendMessage(conversationId:string,bookingId:string,content:string):Promise<ChatMessage> { const data=await request<{message:ChatMessage}>(`/pi/chat/conversations/${encodeURIComponent(conversationId)}/messages`,authBody({bookingId,content})); return data.message; },
