...
export const chatService = {
 ...
 async getReceipts(conversationId:string,messageIds:string[]):Promise<ChatMessageReceipt[]> { const data=await request<{receipts:ChatMessageReceipt[]}>(`/api/pi/chat/conversations/${encodeURIComponent(conversationId)}/receipts/list`,authBody({messageIds})); return data.receipts||[]; },
 ...
 async uploadAttachment(conversationId:string,bookingId:string,file:File,messageId?:string|null):Promise<ChatAttachment> { if(file.size<=0||file.size>10485760)throw new Error('Attachments must be 10 MB or smaller.'); const signed=await request<{path:string;token:string}>(`/pi/chat/conversations/${encodeURIComponent(conversationId)}/attachments/sign`,authBody({bookingId,fileName:file.name,mimeType:file.type,fileSize:file.size})); const uploaded=await supabase.storage.from('chat-attachments').uploadToSignedUrl(signed.path,signed.token,file,{contentType:file.type,cacheControl:'3600'}); if(uploaded.error)throw new Error(uploaded.error.message||'Attachment upload failed.'); const data=await request<{attachment:ChatAttachment}>(`/pi/chat/conversations/${encodeURIComponent(conversationId)}/attachments`,authBody({bookingId,storagePath:signed.path,fileName:file.name,mimeType:file.type,fileSize:file.size,messageId:messageId||null})); return data.attachment; },
};

let backgroundSyncBusy = false;
if (typeof window !== 'undefined') {
  window.setInterval(async () => {
    if (backgroundSyncBusy || !piAuthService.getStoredUser()?.accessToken) return;
    backgroundSyncBusy = true;
    try {
      const conversations = await chatService.getConversations();
      window.dispatchEvent(new CustomEvent('w3c-chat-list-updated', { detail: { conversations } }));
    } catch {} finally { backgroundSyncBusy = false; }
  }, 1000);
}
