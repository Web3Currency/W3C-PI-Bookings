...
export const chatService = {
 ...
 async getMessages(conversationId:string):Promise<ChatMessage[]> { const data=await request<{messages:ChatMessage[]}>(`/pi/chat/conversations/${encodeURIComponent(conversationId)}/messages/list`,authBody()); const messages=data.messages||[]; await chatLocalStore.putMessages(messages.map(toLocalMessage)); return messages; },
