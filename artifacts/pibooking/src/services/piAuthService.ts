...
async function ensureInit(): Promise<boolean> {
  ...
  if (!initPromise) initPromise = Promise.resolve(window.Pi.init({ version: '2.0', sandbox: PI_SANDBOX }));
  try { await initPromise; return true; } catch (e) { console.warn('[Pi] SDK init failed:', e); initPromise = null; return false; }
}

function readWalletAddress(user: any): string | undefined {
  return user?.wallet_address || user?.walletAddress || user?.wallet?.publicKey || user?.wallet?.address || undefined;
}

export const piAuthService = {
  getStoredUser(): PiUser | null {
    try {
      if (sessionStorage.getItem(PI_SCOPE_VERSION_KEY) !== PI_SCOPE_VERSION) {
  ...
  clearStoredUser(): void {
    try { sessionStorage.removeItem(PI_USER_KEY); sessionStorage.removeItem(PI_SCOPE_VERSION_KEY); } catch {}
  },
  ...
  async signIn(): Promise<PiUser> {
    ...
    const auth = await window.Pi.authenticate([...PI_SCOPES], (incompletePayment) => {
      ...
      void (async () => {
        try {
          if (txid) await fetch('/api/pi/payments/complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentId, txid }) });
          await fetch('/api/pi/payments/reconcile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentId }) });
        } catch (err) { console.error('[Pi] Failed to resolve incomplete payment:', err); }
      })();
    });
    if (!auth?.user) throw new Error('Pi authentication returned no user.');
    ...
    const piUser: PiUser = {
      ...
      walletAddress: validated.walletAddress || validated.wallet_address || readWalletAddress(auth.user),
    };
