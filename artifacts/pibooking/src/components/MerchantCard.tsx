  ...
  if (compact) {
    return (
      <div role={onOpenAbout ? 'button' : undefined} tabIndex={onOpenAbout ? 0 : undefined} onClick={openProfile} onKeyDown={(e) => { if (onOpenAbout && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); openProfile(); } }} className={`relative w-full aspect-square rounded-2xl bg-zinc-50 hover:bg-orange-50/60 border border-zinc-200/70 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col text-center overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-orange-500/40 p-3 ${className}`} aria-label={onOpenAbout ? `View ${name} profile` : undefined}>
        ...
        <div className="flex-1 flex items-center justify-end min-h-0 pt-8 pb-3">
          <div className="flex items-center justify-end gap-2.5 w-full pr-1">
            ...
            <div className="min-w-0 flex-1 text-right">
              <div className="flex items-center justify-end gap-1 min-w-0 flex-nowrap">
