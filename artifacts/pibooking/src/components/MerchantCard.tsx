...
export const MerchantCard: React.FC<MerchantCardProps> = ({ merchant, services = [], title, onOpenAbout, actionLabel = 'View Profile', className = '', showBadge = true }) => {
  const [imgError, setImgError] = useState(false); const isBusiness = !!merchant && 'avatarUrl' in merchant; const isProvider = !!merchant && 'fullName' in merchant;
  const rawName = title || (isBusiness ? (merchant as BusinessProfile).name : undefined) || (isProvider ? (merchant as Provider).fullName : undefined); const name = rawName?.trim() || 'Service Merchant';
  const rawPhoto = isBusiness ? (merchant as BusinessProfile).avatarUrl : (isProvider ? (merchant as Provider).photoUrl : undefined); const resolvedPhoto = providerMediaService.getMediaUrl(rawPhoto); const fallbackAvatar = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(name)}&backgroundColor=ea580c,f97316&textColor=ffffff`; const avatarUrl = !imgError && resolvedPhoto ? resolvedPhoto.trim() : fallbackAvatar;
  const headline = (isBusiness ? ((merchant as BusinessProfile).headline || (merchant as BusinessProfile).tagline) : ((merchant as Provider).headline || (merchant as Provider).roleTitle)) || 'Service Provider';
