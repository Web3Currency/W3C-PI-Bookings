    ...
    return (
      ...
      >
        {showBadge && availabilityStatus && (
          <span className={`absolute top-2 left-2 z-10 inline-flex items-center rounded-full px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider ${availabilityStatus === 'online' ? 'bg-emerald-50 text-emerald-700' : 'bg-zinc-100 text-zinc-600'}`}>
            {availabilityStatus}
          </span>
        )}
        {isTestAccount && (
          <span className="absolute top-2 right-2 z-10 rounded-md border border-purple-700 bg-purple-600 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white shadow-sm">TEST</span>
        )}
        <div className="flex-1 flex flex-col items-center justify-center w-full min-h-0 pt-5 pb-2">
          <div className="w-16 h-16 sm:w-[72px] sm:h-[72px] rounded-full bg-orange-100 p-0.5 overflow-hidden shrink-0">
            {avatarUrl ? (
              ...
              <div className="w-full h-full rounded-full flex items-center justify-center text-orange-700 font-black text-xl bg-orange-100">{name.charAt(0).toUpperCase() || '?'}</div>
            )}
          </div>
          <div className="w-full mt-2 px-1 min-h-0">
            ...
            <p className="mt-1 text-[11px] sm:text-xs font-medium text-zinc-500 line-clamp-2 break-words">
              {isProvider ? ((merchant as Provider).headline || (merchant as Provider).roleTitle || 'Service Provider') : 'Business'}
            </p>
          </div>
        </div>
