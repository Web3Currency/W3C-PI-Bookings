  ...
  return (
    <div className="max-w-md mx-auto space-y-4 pb-20 animate-in fade-in duration-200">
      ...
      <div className="p-5 rounded-3xl bg-white shadow-md space-y-5">
        ...
        {paymentStatus === 'success' ? <div className="p-6 text-center space-y-3 bg-emerald-50 rounded-2xl shadow-2xs"><CheckCircle2 className="w-10 h-10 text-emerald-600 mx-auto" /><h4 className="font-extrabold text-zinc-900 text-base">Payment Validated</h4><p className="text-xs text-zinc-600">Opening My Bookings…</p>{simulatedTxHash && <p className="text-xs text-zinc-800 font-mono break-all bg-white p-2.5 rounded-xl shadow-2xs">TxHash: {simulatedTxHash}</p>}</div> : <div className="space-y-4"><div className="p-3.5 rounded-2xl bg-zinc-50 space-y-2 text-xs shadow-2xs"><div className="flex items-center justify-between text-zinc-600"><span>Merchant Receiver:</span><span className="font-extrabold text-zinc-900">{business.name}</span></div><div className="flex items-center justify-between text-zinc-600"><span>Pi Wallet Address:</span><span className="font-mono text-[11px] text-amber-700 font-bold">{business.piWalletAddress}</span></div></div><button onClick={handlePayNow} disabled={paymentStatus === 'processing' || paymentStatus === 'confirming'} id="btn-confirm-and-pay-pi" className="w-full py-3.5 px-5 rounded-2xl bg-amber-600 hover:bg-amber-500 text-white font-black text-sm active:scale-[0.98] transition flex items-center justify-center gap-2 min-h-[48px] shadow-md shadow-amber-600/20 disabled:opacity-50 cursor-pointer">{paymentStatus === 'processing' || paymentStatus === 'confirming' ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Authorizing Pi Blockchain...</span></> : <><Lock className="w-4 h-4 stroke-[2.5]" /><span>Confirm & Pay {totalPayable.toFixed(2)} π</span></>}</button></div>}
      </div>
    </div>
  );
};