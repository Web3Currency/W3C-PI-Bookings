...
import { ArrowLeft, User, ArrowRight, ShieldCheck, AtSign, Mail } from 'lucide-react';
...
export const SelectDetailsStep: React.FC<SelectDetailsStepProps> = ({ service, initialDetails, onBack, onProceedToSummary }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onProceedToSummary({ clientName: clientName.trim(), clientPiUsername: finalHandle, clientPhone: '', clientEmail: clientEmail.trim(), notes });
  };
  return (
    <div className="max-w-md mx-auto space-y-4 pb-28 animate-in fade-in duration-200">
      <BookingProgressBar currentStep={1} />
      <div className="flex items-center justify-between">
        <button type="button" onClick={onBack} id="btn-back-to-service-detail" className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-100 text-zinc-800 text-xs font-bold hover:bg-zinc-200 transition cursor-pointer"><ArrowLeft className="w-3.5 h-3.5" /><span>Service Details</span></button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="p-5 rounded-3xl bg-white shadow-md space-y-4"><h3 className="text-xs font-bold text-zinc-600 uppercase tracking-wider">2. Project Brief</h3><div><label className="block text-[11px] font-bold text-zinc-700 mb-1">Project Brief / Requirements</label><textarea rows={5} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Describe your goals, reference links, specific requirements or questions..." className="w-full p-3 rounded-2xl bg-zinc-50 focus:bg-white text-xs text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition shadow-2xs whitespace-pre-wrap" /></div></div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-3.5 bg-white/95 backdrop-blur-md border-t border-zinc-200 z-50 shadow-lg"><div className="max-w-md mx-auto flex items-center justify-between gap-3"><div className="text-xs"><span className="block text-zinc-400 font-bold text-[10px]">STEP 1 OF 3</span><span className="font-bold text-zinc-900">Contact & Brief</span></div><button type="submit" id="btn-proceed-to-summary" className="flex-1 py-3.5 px-5 rounded-2xl bg-amber-600 hover:bg-amber-500 text-white font-black text-sm active:scale-[0.98] transition flex items-center justify-center gap-2 min-h-[48px] shadow-md shadow-amber-600/20 cursor-pointer"><span>Next: Summary</span><ArrowRight className="w-4 h-4 stroke-[2.5]" /></button></div></div>
      </form>
    </div>
  );
};