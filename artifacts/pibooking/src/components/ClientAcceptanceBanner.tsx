...
export function ClientAcceptanceBanner({ booking, compact = false, onOpenChat }: { booking: Booking; compact?: boolean; onOpenChat?: (bookingId: string) => void }) {
  ...
  if (booking.status === 'Delivered') {
    ...
    if (compact) return <BookingStatusCompact booking={booking} />;
  ...
  if (booking.status === 'In Progress' && booking.project_deadline) {
    ...
    if (compact) return <BookingStatusCompact booking={booking} timer={expired ? 'Expired' : formatProjectCountdown(remaining)} timerLabel="Service time" timerTone={expired ? 'text-rose-600' : 'text-zinc-600'} />;
  ...
  if (compact) return <BookingStatusCompact booking={booking} timer={expired ? 'Expired' : remain} timerLabel="Acceptance" timerTone={expired ? 'text-rose-600' : 'text-zinc-600'} />;
...
function BookingStatusCompact({ booking, timer, timerLabel, timerTone }: { booking: Booking; timer?: string; timerLabel?: string; timerTone?: string }) {
  const badge = clientStatusBadge(booking);
  return <div className="flex items-center justify-end gap-3 leading-none"><span className={`inline-flex items-center gap-1.5 text-[10px] font-extrabold ${badge.classes}`}>{badge.dot && <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${badge.dot}`} />}{badge.label}</span>{timer && <span className={`text-right ${timerTone || 'text-zinc-600'}`}><span className="block text-[8px] font-black uppercase tracking-wider opacity-70">{timerLabel}</span><span className="block mt-0.5 text-[11px] font-black tabular-nums">{timer}</span></span>}</div>;
}
