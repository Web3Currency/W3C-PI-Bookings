  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/95 border-b border-zinc-100 shadow-2xs">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {piAuthLoading ? <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-50 text-orange-950 text-xs font-semibold"><Loader2 className="w-3.5 h-3.5 animate-spin text-orange-600" /><span>Connecting…</span></div> : piUser ? <div className="flex items-center gap-2">{!hasProvider && onOpenBecomeProvider && <button onClick={onOpenBecomeProvider} id="btn-nav-become-provider" title="Become a Provider" className="px-3.5 py-1.5 rounded-full bg-orange-600 hover:bg-orange-500 text-white text-xs font-bold transition shadow-xs cursor-pointer">Become a Provider</button>}<div className="flex items-center gap-1.5 pl-1"><div title={piUser.username} className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-600 to-amber-600 text-white font-black text-xs flex items-center justify-center shadow-2xs">{piUser.username ? piUser.username.charAt(0).toUpperCase() : 'P'}</div></div></div> : <button onClick={onSignIn} id="btn-nav-signin" className="px-4 py-1.5 rounded-full bg-orange-600 hover:bg-orange-500 text-white text-xs font-black transition shadow-2xs cursor-pointer">Sign In</button>}
          </div>
        </div>
      </header>
      ...
      <aside id="side-navigation-drawer" className={`fixed top-0 left-0 bottom-0 w-80 max-w-[85vw] bg-white z-50 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out border-r border-zinc-100 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`} aria-label="Side Navigation Menu">
        ...
        {piUser && onSignOut && <div className="p-3 border-t border-zinc-100 bg-zinc-50"><button onClick={() => handleNavAction(onSignOut)} id="drawer-nav-signout" className="w-full py-2 text-center text-xs font-bold text-zinc-500 hover:text-red-600 transition cursor-pointer">Sign Out of Pi Network</button></div>}
      </aside>
    </>
  );
};
