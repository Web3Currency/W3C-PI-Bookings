...
export const ServiceDetail: React.FC<ServiceDetailProps> = ({ service, business, onBack, onProceedToBooking, onOpenProviderProfile }) => {
  ...
  return <div className="space-y-6 pb-28 animate-in fade-in slide-in-from-right-4 duration-200">
    ...
    {providerName && <div className="p-5 rounded-3xl bg-white shadow-sm flex items-end gap-4"><div className="w-12 h-12 rounded-full overflow-hidden shrink-0 bg-zinc-100">{providerAvatar && <img src={providerAvatar} alt={providerName} className="w-full h-full object-cover" />}</div><div className="min-w-0 flex-1 text-xs self-center"><div className="flex items-center gap-1.5 min-w-0"><div className="font-extrabold text-zinc-900 text-sm truncate">{providerName}</div>{profileVerified && <BadgeCheck className="w-4 h-4 shrink-0 text-orange-600 fill-orange-600 stroke-white" strokeWidth={2.2} title="Verified by W3C Pi Bookings" aria-label="Verified by W3C Pi Bookings" />}</div>{providerRole && <p className="text-orange-600 font-bold mt-0.5">{providerRole}</p>}{resolvedProvider?.piUsername && <p className="text-zinc-400 text-[11px] mt-0.5 font-mono">@{resolvedProvider.piUsername.replace(/^@+/, '')}</p>}</div>{resolvedProvider && onOpenProviderProfile && <button type="button" onClick={() => requireSignIn(() => onOpenProviderProfile(resolvedProvider))} className="shrink-0 text-[11px] font-bold text-orange-700 hover:text-orange-800 whitespace-nowrap self-end">View Profile</button>}</div>}
    <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-zinc-100 z-50 shadow-lg"><div className="max-w-md mx-auto flex items-center justify-between gap-4"><div className="text-xs"><span className="block text-zinc-400 font-bold uppercase text-[10px]">Total Price</span><span className="text-xl font-black text-zinc-900">{service.pricePi} <span className="text-xs font-bold text-orange-600">π</span></span></div><button onClick={() => requireSignIn(onProceedToBooking)} id="btn-proceed-to-schedule" className="px-6 py-3 rounded-2xl bg-orange-600 hover:bg-orange-500 text-white font-black text-xs sm:text-sm transition shadow-sm cursor-pointer">Book Now</button></div></div>
  </div>;
};
