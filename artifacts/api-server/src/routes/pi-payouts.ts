 ...
 *
 * Server-authoritative client refund. Body: { bookingId: string }
 * Amount and client uid are loaded from the booking record — never trusted from the client.
 */
router.post("/pi/payouts/refund", async (req, res) => {
  const { bookingId } = req.body as { bookingId?: string; amountPi?: number; clientPiUid?: string };
  ...
  // Intentionally ignore client-supplied amountPi / clientPiUid.
  try {
    const { executeAutomaticClientRefund } = await import("../services/automaticRefundService");
    const result = await executeAutomaticClientRefund(bookingId.trim(), {
      reason: "Manual/API escrow refund",
      cancelBooking: true,
    });
    ...
    req.log.error({ bookingId, result }, "A2U refund failed");
    res.status(502).json({ success: false, error: result.error || "Refund failed.", ...result });
  } catch (err: any) {
