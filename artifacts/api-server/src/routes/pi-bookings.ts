...
router.post("/pi/bookings/:bookingId/complete", async (req, res) => {
  ...
  try {
    ...
    const supabaseResponse = await supabaseRequest(`bookings?id=eq.${encodeURIComponent(bookingId)}&client_pi_uid=eq.${encodeURIComponent(clientUid)}&status=eq.In%20Progress&escrow_status=eq.paid_escrowed`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ escrow_status: "completion_confirmed", confirmed_at: new Date().toISOString(), updated_at: new Date().toISOString() }) });
    if (supabaseResponse) {
      if (!supabaseResponse.ok) { const text = await supabaseResponse.text().catch(() => ""); throw new Error(text || `Supabase booking completion update failed (${supabaseResponse.status}).`); }
      const rows = await supabaseResponse.json();
      if (!Array.isArray(rows) || rows.length === 0) return void res.status(409).json({ error: "Booking is not in progress, already confirmed, or is not owned by this Pi account." });
      return void res.json({ success: true, booking: rows[0] });
    }
    if (!pool) return void res.status(500).json({ error: "Booking database connection is not configured on the API server." });
    const result = await pool.query(`UPDATE public.bookings SET escrow_status = 'completion_confirmed', confirmed_at = NOW(), updated_at = NOW() WHERE id = $1 AND client_pi_uid = $2 AND status = 'In Progress' AND escrow_status = 'paid_escrowed' RETURNING id, status, escrow_status, confirmed_at, updated_at`, [bookingId, clientUid]);
    if (result.rowCount === 0) return void res.status(409).json({ error: "Booking is not in progress, already confirmed, or is not owned by this Pi account." });
    return void res.json({ success: true, booking: result.rows[0] });
  } catch (err: any) {
