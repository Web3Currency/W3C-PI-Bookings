...
async function supabaseRequest(path: string, init: RequestInit = {}) {
  ...
  if (!config) throw new Error("Booking database configuration is unavailable.");
  ...
  if (!response.ok) throw new Error((await response.text().catch(() => "")) || `Supabase request failed (${response.status}).`);
...
router.post("/pi/bookings/:bookingId/deliver", async (req, res) => {
  ...
  const { accessToken, deliveryNotes, deliveryAttachments } = req.body as {
    accessToken?: string;
    deliveryNotes?: string;
    deliveryAttachments?: Array<Record<string, unknown>>;
  };
  if (!bookingId || !/^[0-9a-f-]{36}$/i.test(bookingId)) return void res.status(400).json({ error: "A valid bookingId is required." });
  const trimmedNotes = String(deliveryNotes || "").trim();
  const attachments = Array.isArray(deliveryAttachments) ? deliveryAttachments.slice(0, 20) : [];
  ...
  try {
    ...
    const rows = await supabaseRequest(`bookings?id=eq.${encodeURIComponent(bookingId)}&select=id,status,provider_id,escrow_status,client_pi_uid,service_title,project_deadline&limit=1`);
    const booking = rows[0];
    if (!booking) return void res.status(404).json({ error: "Booking not found." });
    if (!booking.provider_id) return void res.status(409).json({ error: "Booking has no assigned provider." });

    const providers = await supabaseRequest(`providers?id=eq.${encodeURIComponent(booking.provider_id)}&select=id,pi_uid&limit=1`);
    ...
    try {
      ...
      const existing = await supabaseRequest(`messages?select=id&conversation_id=eq.${encodeURIComponent(conversationId)}&message_type=eq.system&content=eq.${encodeURIComponent(systemContent)}&limit=1`);
      if (!existing[0]) {
      ...
      req.log.error({ chatErr, bookingId }, "Delivery recorded but delivery system message failed");
    }

    return void res.json({ success: true, booking: updated[0] });
  } catch (err: any) {
    req.log.error({ err, bookingId }, "Provider delivery submission failed");
    return void res.status(500).json({ error: err?.message || "Failed to submit delivery." });
  }
});
