...
async function getProviderByPiUid(piUid: string) {
  const response = await supabaseRequest(`providers?select=id,pi_uid,full_name,role_title,status&pi_uid=eq.${encodeURIComponent(piUid)}&limit=1`);
  if (!response) return null;
  if (!response.ok) throw new Error(`Supabase provider lookup failed (${response.status}).`);
  const rows = await response.json() as Array<{ id: string; pi_uid: string; full_name: string; role_title: string | null; status: string }>;
  return rows[0] || null;
}
...
function validateServiceInput(body: Record<string, any>) {
  ...
  const deliverables = Array.isArray(body.deliverables) ? body.deliverables.map((item: unknown) => String(item).trim()).filter(Boolean) : [];
  ...
  return { title, shortDescription, fullDescription, coverImage, deliverables, duration: Math.round(duration), basePriceNgn, category, locationType, status };
}

router.get("/pi/services", async (req, res) => {
  ...
  try {
    ...
    const response = await supabaseRequest(`services?select=*,providers(*)&provider_id=eq.${encodeURIComponent(provider.id)}&order=created_at.desc`);
...
router.post("/pi/services", async (req, res) => {
  const accessToken = String(req.headers.authorization || "").replace(/^Bearer\s+/i, "").trim();
  if (!accessToken) return void res.status(401).json({ error: "Pi access token is required." });
  try {
    const piUser = await verifyPiAccessToken(accessToken);
    ...
    const response = await supabaseRequest("services", { method: "POST", body: JSON.stringify({ title: service.title, short_description: service.shortDescription, full_description: service.fullDescription, cover_image: service.coverImage, deliverables: service.deliverables, duration: service.duration, base_price_ngn: service.basePriceNgn, calculated_pi_price: calculatedPiPrice, status: service.status, category: service.category, provider_name: provider.full_name, provider_role: provider.role_title || "Specialist", provider_id: provider.id, location_type: service.locationType, featured: false }) });
    if (!response) return void res.status(500).json({ error: "Supabase connection is not configured on the API server." });
...
router.patch("/pi/services/:serviceId", async (req, res) => {
  ...
  try {
    ...
    const exchangeRate = await getExchangeRate();
    const calculatedPiPrice = Number((service.basePriceNgn / exchangeRate).toFixed(2));
    const response = await supabaseRequest(`services?id=eq.${encodeURIComponent(serviceId)}&provider_id=eq.${encodeURIComponent(provider.id)}`, { method: "PATCH", body: JSON.stringify({ title: service.title, short_description: service.shortDescription, full_description: service.fullDescription, cover_image: service.coverImage, deliverables: service.deliverables, duration: service.duration, base_price_ngn: service.basePriceNgn, calculated_pi_price: calculatedPiPrice, status: service.status, category: service.category, provider_name: provider.full_name, provider_role: provider.role_title || "Specialist", location_type: service.locationType, updated_at: new Date().toISOString() }) });
    if (!response) return void res.status(500).json({ error: "Supabase connection is not configured on the API server." });
