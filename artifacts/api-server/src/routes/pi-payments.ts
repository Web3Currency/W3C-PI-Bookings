...
async function finalizeBookingFromPayment(
  ...
  if (!providerId && serviceId) {
    try {
      const sres = await sb(`services?id=eq.${encodeURIComponent(String(serviceId))}&select=id,provider_id&limit=1`);
      if (sres?.ok) {
  ...
  // Minimal columns only — avoid unknown columns (PGRST204).
